...
fn main() {
    ...
    if open != close && !content.is_empty() {
        panic!("Brace mismatch in ui/app.slint: {} open, {} close", open, close);
    }
    ...
    for i in 0..unique_names.len() {
        ...
        slint_code.push_str(&format!("        if (current == \"{}\") {{ return \"{}\"; }}\n", current, next));
    }
    ...
    for (i, p) in unique_paths.iter().enumerate() {
        ...
        slint_code.push_str(&format!("icon-name == \"{}\" ? @image-url(\"{}\") : ", name, p));
    }
    ...
    fs::write("ui/generated_icons.slint", slint_code).expect("Failed to write generated_icons.slint");
